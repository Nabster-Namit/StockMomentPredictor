...
class Ui_StockPredictor(object):
    ...
    def retranslateUi(self, StockPredictor):
        ...
        StockPredictor.setWindowTitle(_translate("StockPredictor", "MainWindow"))
    ...
    def sbi_data(self):
        with open(r'C:/Users/namit/PycharmProjects/StockMarket/sbinews.json') as f:
            df = json.load(f)
            data = pd.DataFrame(df['sbi_news'], columns=['date', 'headline'])
            data = data.rename(columns={'date': 'Date', 'headline': 'Headline'})
            data['Date'] = pd.to_datetime(data.Date)
            data.sort_values(by='Date', inplace=True)
            data.reset_index(drop=True, inplace=True)
            data = data.groupby('Date')['Headline'].apply(''.join).reset_index()
            stock_data = pd.read_csv(r'C:\Users\namit\PycharmProjects\StockMarket\Stockdata_SBIN.csv')
        ...
        data.drop(['Headline', 'Compound'], inplace=True, axis=1)
        data.reset_index(drop=True, inplace=True)
    ...
    def reliance_data(self):
        with open(r'C:/Users/namit/PycharmProjects/StockMarket/reliancenews.json') as f:
            df = json.load(f)
            data = pd.DataFrame(df['reliance_news'], columns=['date', 'headline'])
            data = data.rename(columns={'date': 'Date', 'headline': 'Headline'})
            data['Date'] = pd.to_datetime(data.Date)
            data.sort_values(by='Date', inplace=True)
            data.reset_index(drop=True, inplace=True)
    ...
    def tata_data(self):
        with open(r'C:/Users/namit/PycharmProjects/StockMarket/tata_motorsnews.json') as f:
            df = json.load(f)
            data = pd.DataFrame(df['tata_motors_news'], columns=['date', 'headline'])
            data = data.rename(columns={'date': 'Date', 'headline': 'Headline'})
            data['Date'] = pd.to_datetime(data.Date)
            data.sort_values(by='Date', inplace=True)
            data.reset_index(drop=True, inplace=True)
            data = data.groupby('Date')['Headline'].apply(''.join).reset_index()
            stock_data = pd.read_csv(r'C:\Users\namit\PycharmProjects\StockMarket\Stockdata_TATAMOTORS.csv')
            data.set_index('Date', inplace=True)
            stock_data.set_index('Date', inplace=True)
            data = pd.merge(data, stock_data, right_index=True, left_index=True, how='inner')
    ...
    def tcs_data(self):
        with open(r'C:/Users/namit/PycharmProjects/StockMarket/tcsnews.json') as f:
            df = json.load(f)
            data = pd.DataFrame(df['tcs_news'], columns=['date', 'headline'])
            data = data.rename(columns={'date': 'Date', 'headline': 'Headline'})
            data['Date'] = pd.to_datetime(data.Date)
            data.sort_values(by='Date', inplace=True)
            data.reset_index(drop=True, inplace=True)
